...
public class StateSpacePlannerFactory implements Serializable {
    ...
    public static void main(String[] args) {
        try {
            ...
            // call the planner
            plan(planner, errorManager, factory, domain.getName(), problem.getName(), traceLevel, saveStats);

        } catch (FileException | IOException fileEx) {
            LOGGER.error(fileEx);
            System.exit(1);
        }
    }

    public static String plan(Planner.Name plannerName, String domain, String problem, int traceLevel, int timeout, Heuristic.Type heuristicType, double weight, boolean saveStats, boolean actionCost) {
        ...
        try {
            ...
            // call the planner
            return plan(planner, errorManager, factory, "domain.pddl", "problem.pddl", traceLevel, saveStats);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return null;
    ...
    public static String plan(AbstractStateSpacePlanner planner, ErrorManager errorManager, ProblemFactory factory, String domainName, String pName, int traceLevel, boolean saveStats) {
